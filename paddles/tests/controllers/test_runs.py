...
class TestRunController(TestApp):
    ...
    def test_get_not_found(self):
        assert self.app.get("/runs/new_run/", expect_errors=True).status_int == 404

    def test_get_invalid_url_on_run(self):
        assert (
            self.app.get("/runs/new_run/invalid", expect_errors=True).status_int == 404
        )
    ...
    def test_allows_waiting_status(self, job_conf):
        self.app.post_json("/runs/", dict(name="foo"))
        self.app.post_json("/runs/foo/jobs/", job_conf | {"status": "waiting"})
        new_run = Session.scalars(select(Run).where(Run.name == "foo")).one()
        assert new_run.status == "waiting"

    def test_running_status_with_waiting_and_running_jobs(self, job_conf):
        self.app.post_json("/runs/", dict(name="foo"))
        self.app.post_json("/runs/foo/jobs/", job_conf | {"status": "running"})
        self.app.post_json(
    ...
    def test_create_then_get_new_run(self, job_conf):
        self.app.post_json("/runs/", job_conf | {"name": "foo"})
        response = self.app.get("/runs/")
    ...
    def test_create_new_job(self, job_conf):
        self.app.post_json("/runs/", dict(name="foo"))
        self.app.post_json("/runs/foo/jobs/", job_conf | dict(status="queued"))
    ...
    def test_queued_fully(self, job_conf):
    ...
    def test_queued_partially(self, job_conf):
        run_names = ["run_one", "run_two"]
        for run_name in run_names:
            self.app.post_json("/runs/", dict(name=run_name))
            self.app.post_json(
...
class TestRunControllerDateFilters(TestApp):
    ...
    def test_suite_and_branch_and_since(self):
        response = self.app.get("/runs/suite/nfs/branch/next/?since=2013-01-03")
        got_names = sorted(run["name"] for run in response.json)
